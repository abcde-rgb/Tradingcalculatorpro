...
export const STRATEGIES = [
  // ═══ BULLISH (8) ═══
  {
    id: 'long_call', name: 'Long Call · BUY', category: 'Bullish', color: '#22c55e',
    description: 'COMPRA una Call. Pagas prima. Beneficio ilimitado si el subyacente sube por encima de Strike + Prima (break-even).',
    legs: [{ type: 'call', action: 'buy', qty: 1, offset: 0 }],
    risk: 'Limitado (prima pagada)', reward: 'Ilimitado', shape: 'call_buy',
    maxProfit: 'Ilimitado', maxLoss: 'Prima pagada',
    whenToUse: 'IV baja, tendencia alcista fuerte',
  },
  {
    id: 'short_put', name: 'Short Put (CSP) · SELL', category: 'Bullish', color: '#06b6d4',
    description: 'VENDES una Put garantizada con efectivo. Cobras prima. Si cae por debajo del Strike te asignan el stock a precio descontado.',
    legs: [{ type: 'put', action: 'sell', qty: 1, offset: -1 }],
    risk: 'Alto (Strike − Prima si cae a 0)', reward: 'Limitado (prima)', shape: 'put_sell',
    maxProfit: 'Prima cobrada', maxLoss: 'Strike - Prima',
    whenToUse: 'Neutral-alcista, IV elevada, quieres comprar el stock más barato',
  },
  {
  ...
  // ═══ BEARISH (7) ═══
  {
    id: 'long_put', name: 'Long Put · BUY', category: 'Bearish', color: '#ef4444',
    description: 'COMPRAS una Put. Pagas prima. Beneficio si el subyacente cae por debajo de Strike − Prima (break-even). Ideal como seguro.',
    legs: [{ type: 'put', action: 'buy', qty: 1, offset: 0 }],
    risk: 'Limitado (prima pagada)', reward: 'Alto (hasta Strike)', shape: 'put_buy',
    maxProfit: 'Strike - Prima', maxLoss: 'Prima pagada',
    whenToUse: 'IV baja, caída esperada o proteger cartera',
  },
  {
    id: 'short_call', name: 'Short Call (Naked) · SELL', category: 'Bearish', color: '#f97316',
    description: 'VENDES una Call desnuda. Cobras prima. Riesgo ILIMITADO al alza. Requiere margen elevado.',
    legs: [{ type: 'call', action: 'sell', qty: 1, offset: 1 }],
    risk: 'Ilimitado', reward: 'Limitado (prima)', shape: 'call_sell',
    maxProfit: 'Prima cobrada', maxLoss: 'Ilimitado',
    whenToUse: 'Bajista, IV elevada, cuenta con margen alto',
  },
  {
...
export const EDU_MODULES = [
  {
    icon: '◈', title: 'Fundamentos: Comprar vs Vender Opciones', color: '#22c55e',
    content: 'Toda operación con opciones tiene DOS PARTES. El que COMPRA (Long) paga la prima y obtiene un derecho. El que VENDE (Short) cobra la prima y acepta la obligación si se ejerce.',
    items: [
      'LONG CALL (BUY CALL) → Pagas prima. Ganas si sube > Strike+Prima. Pérdida máxima = prima pagada.',
      'LONG PUT (BUY PUT) → Pagas prima. Ganas si baja < Strike−Prima. Pérdida máxima = prima pagada.',
      'SHORT CALL (SELL CALL) → Cobras prima. Pierdes si sube > Strike+Prima. Pérdida máxima = ILIMITADA (si naked).',
      'SHORT PUT (SELL PUT) → Cobras prima. Pierdes si baja < Strike−Prima. Pérdida máxima = Strike−Prima si cae a 0.',
      'REGLA 80/20 → ~80% de las opciones expiran sin valor → estadísticamente el vendedor de primas suele ganar; pero su ratio riesgo/beneficio es malo.',
    ],
  },
  {
    ...
    items: [
      'Delta Δ → Cambio en prima por $1 de movimiento del subyacente. Calls: 0 a +1. Puts: 0 a -1.',
      'Theta Θ → Pérdida de valor temporal por cada día transcurrido. Siempre negativa para Long, positiva para Short.',
      'Vega ν → Sensibilidad al cambio en la volatilidad implícita (IV). Positiva para Long, negativa para Short.',
      'Gamma Γ → Velocidad de cambio del Delta; máximo cerca ATM. Largos: Gamma positivo. Cortos: Gamma negativo.',
      'Rho ρ → Sensibilidad a cambios en tipos de interés. Significativa solo para opciones a largo plazo (LEAPS).',
    ],
  },
  {
    icon: '⬡', title: 'Moneyness: ITM / ATM / OTM (explicado)', color: '#eab308',
    content: 'La MONEYNESS indica la relación entre el precio actual del subyacente (Spot S) y el Strike K. Define el valor intrínseco de la opción y su comportamiento.',
    items: [
      '── CALL ──',
      'ITM (In-The-Money) → S > K. Tiene valor intrínseco = S−K. Delta > 0.5. Comportamiento "como stock".',
      'ATM (At-The-Money) → S ≈ K. Sin valor intrínseco; solo valor temporal. Delta ≈ 0.5. Máxima Gamma y Vega.',
      'OTM (Out-of-The-Money) → S < K. Sin valor intrínseco; solo valor temporal. Delta < 0.5. Más barata pero más riesgosa.',
      '── PUT ──',
      'ITM (In-The-Money) → S < K. Tiene valor intrínseco = K−S. Delta < −0.5.',
      'ATM (At-The-Money) → S ≈ K. Sin valor intrínseco; solo valor temporal. Delta ≈ −0.5.',
      'OTM (Out-of-The-Money) → S > K. Sin valor intrínseco; solo valor temporal. Delta > −0.5.',
      '── CÁLCULO DEL PAYOFF AL VENCIMIENTO ──',
      'Long Call:  PnL = max(0, S−K) × 100 × contratos − prima_pagada × 100',
      'Long Put:   PnL = max(0, K−S) × 100 × contratos − prima_pagada × 100',
      'Short Call: PnL = prima_cobrada × 100 − max(0, S−K) × 100 × contratos',
      'Short Put:  PnL = prima_cobrada × 100 − max(0, K−S) × 100 × contratos',
    ],
  },
  {
    icon: '⌬', title: 'Volatilidad Implícita (IV)', color: '#ef4444',
    content: 'La IV es el precio del mercado para la incertidumbre futura. Una IV alta encarece las primas; una IV baja las abarata. Es el factor MÁS importante tras Delta.',
    items: [
      'IV Rank / IV Percentile → Contextualizan la IV actual vs. histórica (últimas 52 semanas).',
      'Crush post-earnings → La IV colapsa tras el evento; los compradores suelen perder aunque acierten la dirección.',
      'Vega Long vs. Short → Compradores ganan si la IV sube; vendedores si baja.',
      'Regla práctica: COMPRAR opciones con IV baja (IV Rank < 30), VENDER con IV alta (IV Rank > 50).',
      'Skew → Las puts OTM suelen tener mayor IV que las calls OTM (miedo a caídas).',
    ],
  },
  {
    icon: '📐', title: 'Cómo leer el diagrama de Payoff', color: '#a78bfa',
    content: 'El gráfico de payoff muestra el PnL (eje Y) en función del precio del subyacente al vencimiento (eje X). Las dos líneas clave son T+0 (hoy) y At Expiry (vencimiento).',
    items: [
      'Línea VERDE (Current P&L) → Tu PnL teórico HOY a distintos precios usando Black-Scholes (incluye valor temporal).',
      'Línea BLANCA punteada (At Expiry) → PnL SI mantienes hasta el vencimiento (sin valor temporal).',
      'Línea NARANJA (Spot) → Precio actual del subyacente. Referencia principal.',
      'Línea VIOLETA (Break-Even) → Punto donde PnL = 0 al vencimiento. Por encima/debajo ganas.',
      'Zona VERDE → Rango de precios donde terminas con beneficio. Zona ROJA → Pérdidas.',
      'ZONAS DE MONEYNESS (nuevas) → Verde claro = ITM, Amarillo = ATM, Rojo claro = OTM. Ayuda a visualizar dónde está cada strike.',
      'La DIFERENCIA entre ambas líneas = valor extrínseco (tiempo + IV). Theta negativo la reduce cada día.',
    ],
  },
];
