...
const GreekCard = ({ label, value, icon: Icon, color, description }) => (
  <div className="bg-[#0d1117] rounded-lg border border-[#21262d] p-3 hover:border-[#30363d] transition-colors group">
    ...
    <p className="text-[10px] text-[#484f58] mt-1 opacity-0 group-hover:opacity-100 transition-opacity">{description}</p>
  </div>
);
...
const GreeksPanel = ({ greeks, legs, stock }) => {
  ...
  return (
    <div className="p-4">
      <h3 className="text-sm font-bold text-[#f0f6fc] uppercase tracking-wider mb-4">Greeks</h3>
      ...
      {/* Position Summary */}
      <div className="mt-6">
        <h3 className="text-sm font-bold text-[#f0f6fc] uppercase tracking-wider mb-3">Position Summary</h3>
        <div className="bg-[#0d1117] rounded-lg border border-[#21262d] p-3 space-y-3">
          {legs.map((leg, i) => (
          ...
          <div className="border-t border-[#21262d] pt-2 flex items-center justify-between">
            <span className="text-xs text-[#8b949e]">Total Cost</span>
      ...
      {legs[0] && legs[0].type !== 'stock' && (
        <div className="mt-6">
          <h3 className="text-sm font-bold text-[#f0f6fc] uppercase tracking-wider mb-3">Market Data</h3>
          <div className="bg-[#0d1117] rounded-lg border border-[#21262d] p-3 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-[#8b949e]">Implied Volatility</span>
              <span className="text-[#f0f6fc] font-mono">{(legs[0].iv * 100).toFixed(1)}%</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-[#8b949e]">Days to Expiry</span>
              <span className="text-[#f0f6fc] font-mono">{legs[0].daysToExpiry}d</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-[#8b949e]">Risk-Free Rate</span>
              <span className="text-[#f0f6fc] font-mono">5.00%</span>
            </div>
          </div>
        </div>
      )}
