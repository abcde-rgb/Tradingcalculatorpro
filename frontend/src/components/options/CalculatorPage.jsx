...
const CalculatorPage = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0e17] text-[#e1e7ef] overflow-hidden" data-testid="options-calculator-root">
      {/* Top Header */}
      <header className="h-14 min-h-[56px] bg-[#0f1420] border-b border-[#1e2536] flex items-center px-5 gap-4 z-50">
        <div className="flex items-center gap-2.5 mr-4">
          ...
          <span className="text-lg font-bold text-white tracking-tight">Options<span className="text-[#60a5fa]">Pro</span></span>
        </div>
        ...
        {stock && (
          <div className="flex items-center gap-3 ml-2">
            {loading && <Loader2 className="w-4 h-4 text-[#60a5fa] animate-spin" />}
            ...
            <span className="text-[10px] text-[#4a5568] uppercase tracking-wider">{stock.sector}</span>
          </div>
        )}

        <div className="ml-auto flex items-center gap-2">
          <div className="flex bg-[#151c2c] rounded-lg border border-[#253048] overflow-hidden">
            <button onClick={() => setActiveTab('calculator')} className={`px-3.5 py-1.5 text-xs font-medium transition-colors ${activeTab === 'calculator' ? 'bg-[#3b82f6]/15 text-[#60a5fa]' : 'text-[#6b7a94] hover:text-white'}`}>
              <BarChart2 className="w-3.5 h-3.5 inline mr-1" />Calculator
            </button>
            <button onClick={() => setActiveTab('chain')} className={`px-3.5 py-1.5 text-xs font-medium transition-colors ${activeTab === 'chain' ? 'bg-[#3b82f6]/15 text-[#60a5fa]' : 'text-[#6b7a94] hover:text-white'}`}>
              <LayoutGrid className="w-3.5 h-3.5 inline mr-1" />Chain
            </button>
            <button onClick={() => setActiveTab('iv-surface')} className={`px-3.5 py-1.5 text-xs font-medium transition-colors ${activeTab === 'iv-surface' ? 'bg-[#3b82f6]/15 text-[#60a5fa]' : 'text-[#6b7a94] hover:text-white'}`}>
              <Activity className="w-3.5 h-3.5 inline mr-1" />IV Surface
            </button>
            <button onClick={() => setActiveTab('education')} className={`px-3.5 py-1.5 text-xs font-medium transition-colors ${activeTab === 'education' ? 'bg-[#3b82f6]/15 text-[#60a5fa]' : 'text-[#6b7a94] hover:text-white'}`}>
              <BookOpen className="w-3.5 h-3.5 inline mr-1" />Academia
            </button>
          </div>
          <button onClick={() => setShowGuide(true)} className="p-1.5 rounded-lg hover:bg-[#1e2740] transition-colors" title="Guía Rápida">
            <HelpCircle className="w-4 h-4 text-[#4a5568] hover:text-[#eab308]" />
          </button>
      ...
      {activeTab === 'calculator' && (
        <>
          ...
          {builderMode === 'preset' ? (
            ...
            <div className="bg-[#0f1420] border-b border-[#1e2536] px-5 py-2.5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <Wrench className="w-4 h-4 text-[#f59e0b]" />
                  <h3 className="text-sm font-bold text-white">Constructor Multi-Leg</h3>
                </div>
                <span className="text-[10px] text-[#4a5568] bg-[#f59e0b]/10 px-2 py-0.5 rounded-full text-[#fbbf24] font-semibold">
                  {customLegs.filter(l => l.enabled).length} patas
                </span>
              </div>
              <button
                onClick={() => setBuilderMode('preset')}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#3b82f6]/10 border border-[#3b82f6]/25 text-[#60a5fa] text-[10px] font-bold hover:bg-[#3b82f6]/20 transition-all"
              >
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col p-4 gap-3 min-w-0">
              ...
              <div className="grid grid-cols-7 gap-2">
                ...
                <StatCard icon={Percent} label="Prob. Beneficio" value={`${stats.pop || 0}%`} color="text-[#3b82f6]" />
                <StatCard icon={DollarSign} label="Prima Neta" value={`$${stats.premium}`} color={parseFloat(stats.premium) >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'} />
                <StatCard icon={ArrowUpRight} label="ROI" value={`${stats.roi}%`} color="text-[#60a5fa]" />
              </div>
                  ...
                  <div key={`leg-${leg.type}-${leg.action}-${leg.strike}-${i}`} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border ${
                    ...
                    {leg.type === 'stock' ? (
                      ...
                      <>
                        ...
                        <span className="text-[#6b7a94]">@ ${leg.premium?.toFixed(2)}</span>
                      </>
                    )}
                  </div>
                ))}
                <div className="text-xs text-[#6b7a94] ml-2">
                  {currentExp?.fullLabel} · {currentExp?.daysToExpiry}d
                </div>
              </div>

              {/* Chart */}
              <div className="flex-1 bg-[#0f1420] rounded-xl border border-[#1e2536] p-4 min-h-0">
                <PayoffChart
                  data={payoffData}
              ...
              {/* Time Slider */}
              <div className="flex items-center gap-4 bg-[#0f1420] rounded-xl border border-[#1e2536] px-4 py-3">
                <Clock className="w-4 h-4 text-[#6b7a94]" />
                <span className="text-xs text-[#6b7a94] whitespace-nowrap">Tiempo al Vencimiento</span>
                <div className="flex-1 relative">
                  <input type="range" min={0} max={100} value={timeSlider} onChange={(e) => setTimeSlider(parseInt(e.target.value))} className="w-full" />
                </div>
                <span className="text-sm font-mono font-bold text-white min-w-[50px] text-right">{daysForChart}d</span>
                <span className="text-[10px] text-[#4a5568]">/ {currentExp?.daysToExpiry}d</span>
              </div>
            </div>

            {/* Right Panel */}
            <aside className="w-[300px] min-w-[300px] bg-[#0f1420] border-l border-[#1e2536] overflow-y-auto flex flex-col">
              {/* Expiration - always shown */}
              <div className="p-4 border-b border-[#1e2536]">
                <label className="text-[10px] text-[#4a5568] font-semibold uppercase tracking-widest mb-2.5 block">Fecha de Expiración</label>
                  ...
                  {expirations.slice(0, 9).map((exp, idx) => (
                    <button
                      ...
                      className={`px-1.5 py-2 rounded-lg text-center transition-all text-[11px] ${
                        selectedExpIdx === idx
                          ? 'bg-[#3b82f6]/15 text-[#60a5fa] border border-[#3b82f6]/40 font-semibold'
                          : 'bg-[#151c2c] text-[#6b7a94] border border-[#1e2536] hover:border-[#253048] hover:text-[#e1e7ef]'
                      }`}
                    >
              ...
              {builderMode === 'custom' ? (
                ...
                <>
                  {/* Strike Price */}
                  <div className="p-4 border-b border-[#1e2536]">
                    <label className="text-[10px] text-[#4a5568] font-semibold uppercase tracking-widest mb-2.5 block">Precio Strike</label>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setSelectedStrikeIdx(Math.max(0, selectedStrikeIdx - 1))} className="w-8 h-8 rounded-lg bg-[#151c2c] border border-[#253048] flex items-center justify-center hover:bg-[#1e2740] hover:border-[#3b82f6]/40 transition-all">
                        <Minus className="w-3.5 h-3.5" />
                      </button>
                      <div className="flex-1 bg-[#151c2c] border border-[#253048] rounded-lg px-3 py-2 text-center">
                        <span className="text-lg font-bold text-white font-mono">${selectedStrike?.strike || '—'}</span>
                      </div>
                      <button onClick={() => setSelectedStrikeIdx(Math.min(chain.length - 1, selectedStrikeIdx + 1))} className="w-8 h-8 rounded-lg bg-[#151c2c] border border-[#253048] flex items-center justify-center hover:bg-[#1e2740] hover:border-[#3b82f6]/40 transition-all">
                      ...
                      {chain.map((s, idx) => (
                        <button key={s.strike} onClick={() => setSelectedStrikeIdx(idx)}
                          className={`w-full flex items-center justify-between px-2.5 py-1 rounded text-[11px] font-mono transition-all ${
                            selectedStrikeIdx === idx ? 'bg-[#3b82f6]/15 text-[#60a5fa]'
                            : s.strike <= (stock?.price || 0) ? 'text-[#4ade80]/70 hover:bg-[#1e2740]'
                            : 'text-[#f87171]/70 hover:bg-[#1e2740]'
                          }`}
                        >
                          <span>${s.strike}</span>
                          <div className="flex gap-3 text-[#6b7a94]">
                  ...
                  {/* Contracts */}
                  <div className="p-4 border-b border-[#1e2536]">
                    <label className="text-[10px] text-[#4a5568] font-semibold uppercase tracking-widest mb-2.5 block">Contratos</label>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setContracts(Math.max(1, contracts - 1))} className="w-8 h-8 rounded-lg bg-[#151c2c] border border-[#253048] flex items-center justify-center hover:bg-[#1e2740] transition-all">
                        <Minus className="w-3.5 h-3.5" />
                      </button>
                      <input
                        type="number" value={contracts} min={1}
                        onChange={(e) => setContracts(Math.max(1, parseInt(e.target.value) || 1))}
                        className="flex-1 bg-[#151c2c] border border-[#253048] rounded-lg px-3 py-2 text-center text-white font-bold font-mono focus:outline-none focus:border-[#3b82f6]"
                      />
                      <button onClick={() => setContracts(contracts + 1)} className="w-8 h-8 rounded-lg bg-[#151c2c] border border-[#253048] flex items-center justify-center hover:bg-[#1e2740] transition-all">
                        <Plus className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="bg-[#0f1420] rounded-lg border border-[#1e2536] px-2.5 py-2.5 hover:border-[#253048] transition-colors">
    <div className="flex items-center gap-1 mb-0.5">
      <Icon className={`w-3 h-3 ${color}`} />
      <span className="text-[9px] text-[#4a5568] uppercase tracking-wider font-medium truncate">{label}</span>
    </div>
